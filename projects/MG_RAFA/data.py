...
random_erasing = Temporal_erasing(mean=[0.485, 0.456, 0.406])
train_transform = T.Compose(
    [T.Resize((256, 128)), T.ToTensor(), T.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])
...
class Video_train_Dataset(Dataset):
    ...
    def __getitem__(self, ID):
        ...
        ##返回(序列数，序列帧数，3，H,W)
        images = torch.stack(one_id_tracks, dim=0)
        return images, ID * torch.ones(self.track_per_class, dtype=torch.int64)
...
def Get_Video_train_DataLoader(db_txt, info, transform, shuffle=True, num_workers=4, S=6, track_per_class=4,
                               class_per_batch=12,distribute=True):
    ...
    dataloader = DataLoader(dataset, batch_size=class_per_batch, collate_fn=Video_train_collate_fn, shuffle=(sampler is None),sampler=sampler,
                             worker_init_fn=worker_init_fn,drop_last=True, num_workers=num_workers)
...
def Get_Video_test_DataLoader(db_txt, info, query, transform, batch_size=96, shuffle=False, num_workers=4, S=6,
                              distractor=True,distribute=True):
    dataset = Video_test_Dataset(db_txt, info, query, transform, S, distractor=distractor)


    dataloader = DataLoader(dataset, batch_size=batch_size, collate_fn=Video_test_collate_fn, shuffle=False,
