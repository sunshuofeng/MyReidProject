...
##需要注意这个与baseline的分类损失不一样 将logsoftmax改为log了，softmax的过程在模型里做了，请查看论文以及模型代码
...
class Criterion(nn.Module):
    def __init__(self, num_class):
        ...
        self.metric_criterion = TripletLoss(0.5)

    def forward(self, out, label):
        loss = 0
        loss = loss + self.cls_criterion(out['logit'], label)
        loss = loss + self.metric_criterion(out['feature'], label)
        #         loss=loss+0.1*torch.sum(torch.norm(out['xd'],p=2,dim=[1,2]))
        return loss